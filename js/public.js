...
function pupFunShow(showLayer){
    $(".pup").fadeIn();
    $("#"+showLayer).css({"opacity": 1});
}

function pupFunHide(){
    $(".pupcon").css({"opacity": 0});
    $(".pup").fadeOut();
}



if($(window).height() > 450 && $(window).height() != 519){
...
///weixin/jssdk?url=www.baidu.com
...
function wechatShare(appid_val, timestamp_val, nonceStr_val, signature_val){
  ...
  wx.ready(function(){
    ...
    wx.onMenuShareTimeline({
        ...
        success: function () {
            // 用户确认分享后执行的回调函数
            _hmt.push(['_trackEvent', 'shareTimeline', '朋友圈'+myUrl.relative]);
    ...
    wx.onMenuShareAppMessage({
        ...
        success: function () { 
            // 用户确认分享后执行的回调函数
            _hmt.push(['_trackEvent', 'shareAppMessage', '朋友'+myUrl.relative]);
            shareData.shareCallback();
            //alert('分享成功');
        },
        cancel: function () { 
